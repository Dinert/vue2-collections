 /*
     ...
     // step:几个阶级（几步）
   */
  function GradientColarRGBA(startColor, endColor, step, opacity = 1) {
  ...
  function GradientColor(startColor, endColor, step, opacity = 1) {
     var  colorArr = new GradientColarRGBA(startColor, endColor, step, opacity);
     var  result = [];
     for (let i = 0; i < step; i++) {
         // 计算每一步的hex值
         // eslint-disable-next-line radix
  ...
  * 根据变化率颜色变化
  */
  function getGradientColorByRatio(ratio) {
     var  colorbar = ['#43ce17', '#efdc31', '#fa0', '#ff401a', '#d20040', '#9c0a4e'];
  ...
  * 根据变化率颜色变化
  */
  function getGradientColorByAqi(ratio) {
     var  colorbar = ['#43ce17', '#efdc31', '#fa0', '#ff401a', '#d20040', '#9c0a4e'];
     // eslint-disable-next-line one-var
     var  color,
         bgcolor,
