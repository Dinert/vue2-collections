...
const createCylinder = function (map, options) { // 创建圆柱
    const defaultOptions = {
        ...
        style: {
            ...
            radius: 20,
            height: function (res) {
                var value = (res.value[options.type] * 100) || 0;
                return value;
            },

            // 顶边颜色
            color: function (res) {
                var value = res.value[options.type] || 0;

                var color = getAir(options.type, value).color
                return color;
            }
        },
        selectStyle: {
    ...
    return new Promise(function (resolve, reject){
        ...
        var scatterPointLayer = new Loca.ScatterPointLayer({
            map: map,
            eventSupport: true,
        });
