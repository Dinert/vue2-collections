...
BackWardTrack.prototype.createLegend = function () {
    ...
    for (const StationId in this.options.dataBuffer) {
        const itemData = this.options.dataBuffer[StationId]
...
BackWardTrack.prototype.createPoint = function () {
    for (const StationId in this.options.dataBuffer) {
        ...
        // 绘制起始点
        for (let i = 0; i < itemData.length; i++) {
            const name = itemData[i] && itemData[i][0].TimePoint.split(':')[0]
            this.options.config.series.push({
                name: name,
                type: 'effectScatter',
                mapType: 'china',
...
BackWardTrack.prototype.createLine = function () {
    for (const StationId in this.options.dataBuffer) {
        ...
        for (let i = 0; i < itemData.length; i++) {
            const lineItem = []
            // var lineData = [];
            for (let j = itemData[i].length - 1; j >= 0; j--) {
